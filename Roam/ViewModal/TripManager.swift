...
class TripManager: ObservableObject {
    ...
    func getTripRange() -> ClosedRange<Date> {
        return Calendar.current.startOfDay(for: trip.startDate ?? Date()) ... Calendar.current.startOfDay(for: Calendar.current.date(byAdding: .day, value: 1, to: trip.endDate ?? Date()) ?? Date())
    }
